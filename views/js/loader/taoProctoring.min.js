...
html:listBoxActionsTpl({id:config.deliveryExecution,cancelable:config.cancelable})}],$content=(listBox({title:"",textEmpty:"",textNumber:"",textLoading:"",renderTo:$container,list:boxes,width:12}),$container.find(".listbox .content")),deliveryStarted=!1,runDelivery=function(){loadingBar.start(),function(){var selected,textAreaToSelContent;if(window.clipboardData)window.clipboardData.setData("Text"," ");else{if((textAreaToSelContent=document.createElement("textarea")).setAttribute("id","clipboardCleanerPlugin"),textAreaToSelContent.value=" ",textAreaToSelContent.setAttribute("readonly",""),textAreaToSelContent.style.position="absolute",
textAreaToSelContent.style.left="-9999px",document.body.appendChild(textAreaToSelContent),selected=0<document.getSelection().rangeCount&&document.getSelection().getRangeAt(0),textAreaToSelContent.select(),!document.execCommand("copy"))throw new Error("Clipboard can not be cleaned");document.body.removeChild(textAreaToSelContent),selected&&(document.getSelection().removeAllRanges(),document.getSelection().addRange(selected))}}(),deliveryStarted=!0,window.location.href=runDeliveryUrl},isRunnable=function(){return!$container.hasClass("authorization-in-progress")&&!deliveryStarted};function exit(){window.location.href=config.returnUrl}keepAfterResume().reset(),
loadingBar.start(!1),$container.on("click",".js-cancel",function(e){e.stopPropagation(),e.preventDefault(),dialogConfirm(__("Are you sure you want to end the test?"),function(){window.location.href=config.cancelUrl})}),$container.on("click",".js-proceed",function(e){return isRunnable()&&runDelivery(),!1}),$container.on("click",".block.box",function(e){return isRunnable()&&runDelivery(),!1}),polling({action:function(){var async=this.async();$.get(isAuthorizedUrl,function(result){var stop=!1;result.success?result.authorized&&(stop=!0,$content.html(authSuccessTpl({message:__("Authorized, you may proceed")})),loadingBar.stop(),$container.addClass(
"authorization-granted").removeClass("authorization-in-progress")):(stop=!0,result.message?dialogAlert(result.message,exit):exit()),stop?async.reject():async.resolve()})},interval:1e4,autoStart:!0}).next()}}}),define("taoProctoring/controller/Irregularity/index",["jquery","lodash","i18n","ui/hider","util/url","ui/feedback","core/taskQueue/taskQueue","ui/taskQueueButton/standardButton"],function($,_,__,hider,urlHelper,feedback,taskQueue,taskCreationButtonFactory){"use strict";return{start:function(){var taskCreationButton,purpose,$header=$("header.section-header"),$formContainer=$(".print-form"),$reportContainer=$(".print-report"),$form=$("form",
$formContainer),$submitter=$(".form-submitter",$form),$containers=$(".main-container");function refreshTree(){$(".tree").trigger("refresh.taotree",[{uri:$header.data("select-node")}])}purpose="form",hider.hide($containers),hider.show($containers.filter('[data-purpose="'+purpose+'"]')),taskCreationButton=taskCreationButtonFactory({type:"info",icon:"export",title:__("Export Irregularities"),label:__("Export"),taskQueue:taskQueue,taskCreationUrl:$form.prop("action"),taskCreationData:function(){return $form.serializeArray()},taskReportContainer:$reportContainer}).on("finished",function(result){result.task&&result.task.report&&_.isArray(
result.task.report.children)&&result.task.report.children.length&&result.task.report.children[0]&&(result.task.report.children[0].data&&result.task.report.children[0].data.uriResource?(feedback().info(__("%s completed",result.task.taskLabel)),refreshTree(result.task.report.children[0].data.uriResource)):this.displayReport(result.task.report.children[0],__("Error")))}).on("continue",function(){refreshTree()}).on("error",function(err){feedback().error(err)}).render($submitter.closest(".form-toolbar")),$submitter.replaceWith(taskCreationButton.getElement().css({float:"right"}))}}}),define("tpl!taoProctoring/component/dateRange/form",["handlebars"],function(hb){
return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing,escapeExpression=this.escapeExpression;return buffer+='<form class="daterange">\n    <label>'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"From",options):helperMissing.call(depth0,"__","From",options)))+'</label><input type="text" name="periodStart" value="',buffer+=escapeExpression((helper=helpers.start)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(
helper=depth0&&depth0.start)?helper.call(depth0,{hash:{},data:data}):helper)+'"/>\n    <label>'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"to",options):helperMissing.call(depth0,"__","to",options)))+'</label><input type="text" name="periodEnd" value="',buffer+=escapeExpression((helper=helpers.end)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.end)?helper.call(depth0,{hash:{},data:data}):helper)+'"/>\n    <button class="small info" data-control="filter"><span class="icon icon-filter"></span>'+escapeExpression((options={hash:{},data:data},(
