...
var values=value.split(" - "),result="",dateFormat=locale.getDateTimeFormat();return values[0]&&(result=moment(values[0],dateFormat).format("X")),values[1]&&(values[1]=moment(values[1],dateFormat).add(1,"d").format("X"),""!==result&&(result+=" - "),result+=values[1]),result},customFilter:{template:'<input type="text" id="start_time_filter" name="filter[start_time]"/><button class="icon-find js-start_time_filter_button" type="button"></button>',callback:function($el){var dateFormatStr=locale.getDateTimeFormat().split(" ")[0];dateFormatStr=(dateFormatStr=(dateFormatStr=dateFormatStr.replace("YYYY","yy")).replace("MM","mm")).replace("DD","dd"),$el.datepicker({
dateFormat:dateFormatStr,onSelect:function(selectedDate){$(this).data().datepicker.first?(selectedDate>$(this).data().datepicker.first?$(this).val($(this).data().datepicker.first+" - "+selectedDate):$(this).val(selectedDate+" - "+$(this).data().datepicker.first),$(this).data().datepicker.inline=!1,$(".js-start_time_filter_button").trigger("click")):($(this).data().datepicker.inline=!0,$(this).data().datepicker.first=selectedDate)},onClose:function(){delete $(this).data().datepicker.first,$(this).data().datepicker.inline=!1}})}}}),model.push({id:"status",label:__("Status"),sortable:!0,filterable:!0,customFilter:{template:statusFilterTpl({
statuses:_status.getStatuses()}),callback:statusFilterHandler},transform:function(value,row){var status,result="";return row&&row.state&&row.state.status&&(status=_status.getStatusByCode(row.state.status))&&(result=status.label,"INPROGRESS"===row.state.status&&(result=status.label),"Awaiting"===result&&highlightRows.push(row.id)),result}}),model.push({id:"authorizeCl",label:__("Authorize"),type:"actions",actions:[{id:"authorize",icon:"play",title:__("Authorize session"),disabled:function(){return!canDo("authorize",this.state)},action:authorize}]}),(null===data.canPause||data.canPause)&&model.push({id:"pauseCl",label:__("Pause"),type:"actions",actions:[{
...
setTagUsage(applyTags=!!_.isUndefined(applyTags)||applyTags)),highlightRows.length&&_.forEach(highlightRows,function(v){$list.datatable("highlightRow",v)}),loadingBar.stop(),appController.off("change.polling").on("change.polling",function(){polling.stop()}),polling.start(),timer.resume()}).on("select.datatable",function(){actionButtons.each(function($btn){$btn.hide()}),_($list.datatable("selection")).map(function(uri){return getExecutionData(uri).state.status}).uniq().each(function(statusCode){var status=_status.getStatusByCode(statusCode);actionButtons.forIn(function($btn,action){status&&!0===status.can[action]&&$btn.show()})})}).on("error.datatable",
function(e,err){appController.onError(err)}).datatable({url:urlHelper.build(executionsUrl,serviceParams),status:{empty:__("No sessions"),available:function(){return $list.data("applytags")?__("Groups: %s. %s",defaultTag.split(",").join(", "),defaultAvailableLabel):defaultAvailableLabel},loading:__("Loading")},filterStrategy:"multiple",filterSelector:"select, input:not(.select2-input, .select2-focusser)",filter:!0,tools:tools,model:model,selectable:!0,sortorder:"desc",sortby:"start_time"},dataset),now=new Date,nowStr=(nowStr=(nowStr=(nowStr=locale.getDateTimeFormat().split(" ")[0]).replace("YYYY",now.getFullYear())).replace("MM",("0"+(now.getMonth()+1)
).slice(-2))).replace("DD",("0"+now.getDate()).slice(-2)),$("#start_time_filter").val(nowStr+" - "+nowStr),defaultTag&&setTagUsage(!0),$list.datatable("filter")})}).catch(function(err){appController.onError(err),loadingBar.stop()})}}}),define("taoProctoring/controller/Delivery/testTakers",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","taoProctoring/component/breadcrumbs","ui/datatable"],function($,__,helpers,loadingBar,encode,feedback,breadcrumbsFactory){"use strict";return loadingBar.start(),{start:function(){var $container=$(".delivery-testtakers"),$list=$container.find(".list"),crumbs=$container.data("breadcrumbs"),
dataset=$container.data("set"),deliveryId=$container.data("delivery"),testCenterId=$container.data("testcenter"),serviceUrl=helpers._url("availableTestTakers","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),assignUrl=helpers._url("assignTestTakers","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),managerUrl=helpers._url("manage","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),assign=(breadcrumbsFactory($container,crumbs),function(selection){selection&&selection.length&&(loadingBar.start(),$.ajax({url:assignUrl,data:{testtaker:selection},dataType:"json",type:"POST",error:function(){
loadingBar.stop()}}).done(function(response){loadingBar.stop(),response&&response.success?(feedback().success(__("Test takers have been added")),window.location.href=managerUrl):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})}))});$list.on("query.datatable",function(){loadingBar.start()}).on("load.datatable",function(event,response){loadingBar.stop()}).datatable({url:serviceUrl,status:{empty:__("No available test takers to assign"),available:__("Available test takers"),loading:__("Loading")},tools:[{id:"back",icon:"left",title:__("Return to the session manager"),label:__("Back"),action:function(){
window.location.href=managerUrl}},{id:"refresh",icon:"reset",title:__("Refresh the page"),label:__("Refresh"),action:function(){$list.datatable("refresh")}},{id:"assign",icon:"add",title:__("Assign the selected test takers to the session"),label:__("Assign the selected test takers"),massAction:!0,action:function(selection){assign(selection)}}],actions:[{id:"assign",icon:"add",title:__("Assign the test taker to the session"),action:function(id){assign([id])}}],selectable:!0,model:[{id:"firstname",label:__("First name")},{id:"lastname",label:__("Last name")},{id:"identifier",label:__("Identifier")}]},dataset)}}}),define(
"tpl!taoProctoring/templates/deliveryServer/authorizationSuccess",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="";return buffer+='<span class="success-message">',buffer+=(0,this.escapeExpression)((helper=helpers.message)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.message)?helper.call(depth0,{hash:{},data:data}):helper)+'<span class="icon-result-ok"></span></span>'})}),define("tpl!taoProctoring/templates/deliveryServer/authorizationListBoxActions",["handlebars"
],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var stack1,helper,options,buffer="",helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing,escapeExpression=this.escapeExpression;return buffer+='<span class="listbox-actions">\n    ',((stack1=helpers.if.call(depth0,depth0&&depth0.cancelable,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var helper,options,buffer="";return buffer+='\n    <span class="action cancel js-cancel">\n        <span class="icon-stop"></span>'+escapeExpression((options={hash:{},data:data},(
helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Cancel",options):helperMissing.call(depth0,"__","Cancel",options)))+"\n    </span>\n    "},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    <span class="action play js-proceed" data-delivery="',stack1=(helper=helpers.id)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.id)?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'">\n        <span class="icon-play"></span>'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Proceed",options):helperMissing.call(depth0,"__",
"Proceed",options)))+"\n    </span>\n</span>"})}),define("taoProctoring/controller/DeliveryServer/awaiting",["lodash","jquery","i18n","helpers","layout/loading-bar","ui/listbox","ui/dialog/alert","core/polling","taoQtiTest/testRunner/resumingStrategy/keepAfterResume","util/url","ui/dialog/confirm","tpl!taoProctoring/templates/deliveryServer/authorizationSuccess","tpl!taoProctoring/templates/deliveryServer/authorizationListBoxActions"],function(_,$,__,helpers,loadingBar,listBox,dialogAlert,polling,keepAfterResume,url,dialogConfirm,authSuccessTpl,listBoxActionsTpl){"use strict";return{start:function(config){var $container=$(".awaiting-authorization"),
isAuthorizedUrl=helpers._url("isAuthorized","DeliveryServer","taoProctoring",{deliveryExecution:config.deliveryExecution}),runDeliveryUrl=config.runDeliveryUrl,boxes=[{id:"goToDelivery",label:config.deliveryLabel,url:runDeliveryUrl,content:__("Please wait, authorization in process ..."),html:listBoxActionsTpl({id:config.deliveryExecution,cancelable:config.cancelable})}],$content=(listBox({title:"",textEmpty:"",textNumber:"",textLoading:"",renderTo:$container,list:boxes,width:12}),$container.find(".listbox .content")),deliveryStarted=!1,runDelivery=function(){loadingBar.start(),deliveryStarted=!0,window.location.href=runDeliveryUrl},isRunnable=function(){
return!$container.hasClass("authorization-in-progress")&&!deliveryStarted};function exit(){window.location.href=config.returnUrl}keepAfterResume().reset(),loadingBar.start(!1),$container.on("click",".js-cancel",function(e){e.stopPropagation(),e.preventDefault(),dialogConfirm(__("Are you sure you want to end the test?"),function(){window.location.href=config.cancelUrl})}),$container.on("click",".js-proceed",function(e){return isRunnable()&&runDelivery(),!1}),$container.on("click",".block.box",function(e){return isRunnable()&&runDelivery(),!1}),polling({action:function(){var async=this.async();$.get(isAuthorizedUrl,function(result){var stop=!1
;result.success?result.authorized&&(stop=!0,$content.html(authSuccessTpl({message:__("Authorized, you may proceed")})),loadingBar.stop(),$container.addClass("authorization-granted").removeClass("authorization-in-progress")):(stop=!0,result.message?dialogAlert(result.message,exit):exit()),stop?async.reject():async.resolve()})},interval:1e4,autoStart:!0}).next()}}}),define("taoProctoring/controller/Irregularity/index",["jquery","lodash","ui/hider","util/url","report","ui/taskQueue/table","jquery.fileDownload"],function($,_,hider,urlHelper,report,taskQueueTableFactory){"use strict";return{start:function(){var $header=$("header.section-header"),$formContainer=$(
".print-form"),$reportContainer=$(".print-report"),$form=$("form",$formContainer),$submitter=$(".form-submitter",$form),$sent=$(":input[name='"+$form.attr("name")+"_sent']",$form),asyncQueue=$header.data("async-queue"),$containers=$(".main-container");function switchContainer(purpose){hider.hide($containers),hider.show($containers.filter('[data-purpose="'+purpose+'"]'))}function refreshTree(){$(".tree").trigger("refresh.taotree",[{uri:$header.data("select-node")}])}function displayReport(response){switchContainer("report"),$reportContainer.append(response),hider.toggle($("#fold",$reportContainer),response.nested),$('#fold > input[type="checkbox"]',
$reportContainer).on("click",function(){report.fold()}),$("#import-continue",$reportContainer).on("click",refreshTree)}switchContainer("form"),$submitter.off("click").on("click",function(e){var params={},instances=[],classes=[];e.preventDefault(),parseInt($sent.val(),10)&&(_.forEach($form.serializeArray(),function(param){0===param.name.indexOf("instances_")?instances.push(param.value):0===param.name.indexOf("classes_")?classes.push(param.value):params[param.name]=param.value}),asyncQueue?$.ajax({url:$form.attr("action"),data:params,type:"POST",dataType:"text"}).done(displayReport):$.fileDownload($form.attr("action"),{httpMethod:"POST",data:params,
failCallback:displayReport,successCallback:refreshTree}))}),asyncQueue&&taskQueueTableFactory({rows:10,context:$header.data("queue"),dataUrl:urlHelper.route("getTasks","TaskQueueData","tao"),statusUrl:urlHelper.route("getStatus","TaskQueueData","tao"),removeUrl:urlHelper.route("archiveTask","TaskQueueData","tao"),downloadUrl:urlHelper.route("downloadTask","TaskQueueData","tao")}).init().render($containers.filter(".print-tasks"))}}}),define("tpl!taoProctoring/component/dateRange/form",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",
helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing,escapeExpression=this.escapeExpression;return buffer+='<form class="daterange">\n    <label>'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"From",options):helperMissing.call(depth0,"__","From",options)))+'</label><input type="text" name="periodStart" value="',buffer+=escapeExpression((helper=helpers.start)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.start)?helper.call(depth0,{hash:{},data:data}):helper)+'"/>\n    <label>'+escapeExpression((options={hash:{},data:data},(
helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"to",options):helperMissing.call(depth0,"__","to",options)))+'</label><input type="text" name="periodEnd" value="',buffer+=escapeExpression((helper=helpers.end)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.end)?helper.call(depth0,{hash:{},data:data}):helper)+'"/>\n    <button class="small info" data-control="filter"><span class="icon icon-filter"></span>'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Filter",options):helperMissing.call(depth0,"__","Filter",options)))+"</button>\n</form>"})}),define(
"jquery.timePicker",["jquery","jqueryui"],function(jQuery){var $=jQuery;if($.ui.timepicker=$.ui.timepicker||{},!$.ui.timepicker.version){$.extend($.ui,{timepicker:{version:"1.3"}});var Timepicker=function(){this.regional=[],this.regional[""]={currentText:"Now",closeText:"Done",amNames:["AM","A"],pmNames:["PM","P"],timeFormat:"HH:mm",timeSuffix:"",timeOnlyTitle:"Choose Time",timeText:"Time",hourText:"Hour",minuteText:"Minute",secondText:"Second",millisecText:"Millisecond",microsecText:"Microsecond",timezoneText:"Time Zone",isRTL:!1},this._defaults={showButtonPanel:!0,timeOnly:!1,showHour:null,showMinute:null,showSecond:null,showMillisec:null,
showMicrosec:null,showTimezone:null,showTime:!0,stepHour:1,stepMinute:1,stepSecond:1,stepMillisec:1,stepMicrosec:1,hour:0,minute:0,second:0,millisec:0,microsec:0,timezone:null,hourMin:0,minuteMin:0,secondMin:0,millisecMin:0,microsecMin:0,hourMax:23,minuteMax:59,secondMax:59,millisecMax:999,microsecMax:999,minDateTime:null,maxDateTime:null,onSelect:null,hourGrid:0,minuteGrid:0,secondGrid:0,millisecGrid:0,microsecGrid:0,alwaysSetTime:!0,separator:" ",altFieldTimeOnly:!0,altTimeFormat:null,altSeparator:null,altTimeSuffix:null,pickerTimeFormat:null,pickerTimeSuffix:null,showTimepicker:!0,timezoneList:null,addSliderAccess:!1,sliderAccessArgs:null,
controlType:"slider",defaultValue:null,parse:"strict"},$.extend(this._defaults,this.regional[""])};$.extend(Timepicker.prototype,{$input:null,$altInput:null,$timeObj:null,inst:null,hour_slider:null,minute_slider:null,second_slider:null,millisec_slider:null,microsec_slider:null,timezone_select:null,hour:0,minute:0,second:0,millisec:0,microsec:0,timezone:null,hourMinOriginal:null,minuteMinOriginal:null,secondMinOriginal:null,millisecMinOriginal:null,microsecMinOriginal:null,hourMaxOriginal:null,minuteMaxOriginal:null,secondMaxOriginal:null,millisecMaxOriginal:null,microsecMaxOriginal:null,ampm:"",formattedDate:"",formattedTime:"",formattedDateTime:"",
timezoneList:null,units:["hour","minute","second","millisec","microsec"],support:{},control:null,setDefaults:function(settings){return extendRemove(this._defaults,settings||{}),this},_newInst:function($input,o){var tp_inst=new Timepicker,inlineSettings={},fns={},overrides,i;for(var attrName in this._defaults)if(this._defaults.hasOwnProperty(attrName)){var attrValue=$input.attr("time:"+attrName);if(attrValue)try{inlineSettings[attrName]=eval(attrValue)}catch(err){inlineSettings[attrName]=attrValue}}for(i in overrides={beforeShow:function(input,dp_inst){if($.isFunction(tp_inst._defaults.evnts.beforeShow))return tp_inst._defaults.evnts.beforeShow.call($input[0]
,input,dp_inst,tp_inst)},onChangeMonthYear:function(year,month,dp_inst){tp_inst._updateDateTime(dp_inst),$.isFunction(tp_inst._defaults.evnts.onChangeMonthYear)&&tp_inst._defaults.evnts.onChangeMonthYear.call($input[0],year,month,dp_inst,tp_inst)},onClose:function(dateText,dp_inst){!0===tp_inst.timeDefined&&""!==$input.val()&&tp_inst._updateDateTime(dp_inst),$.isFunction(tp_inst._defaults.evnts.onClose)&&tp_inst._defaults.evnts.onClose.call($input[0],dateText,dp_inst,tp_inst)}},overrides)overrides.hasOwnProperty(i)&&(fns[i]=o[i]||null);tp_inst._defaults=$.extend({},this._defaults,inlineSettings,o,overrides,{evnts:fns,timepicker:tp_inst}),
tp_inst.amNames=$.map(tp_inst._defaults.amNames,function(val){return val.toUpperCase()}),tp_inst.pmNames=$.map(tp_inst._defaults.pmNames,function(val){return val.toUpperCase()}),tp_inst.support=detectSupport(tp_inst._defaults.timeFormat+(tp_inst._defaults.pickerTimeFormat?tp_inst._defaults.pickerTimeFormat:"")+(tp_inst._defaults.altTimeFormat?tp_inst._defaults.altTimeFormat:"")),"string"==typeof tp_inst._defaults.controlType?("slider"==tp_inst._defaults.controlType&&void 0===jQuery.ui.slider&&(tp_inst._defaults.controlType="select"),tp_inst.control=tp_inst._controls[tp_inst._defaults.controlType]):tp_inst.control=tp_inst._defaults.controlType
;var timezoneList=[-720,-660,-600,-570,-540,-480,-420,-360,-300,-270,-240,-210,-180,-120,-60,0,60,120,180,210,240,270,300,330,345,360,390,420,480,525,540,570,600,630,660,690,720,765,780,840];null!==tp_inst._defaults.timezoneList&&(timezoneList=tp_inst._defaults.timezoneList);var tzl=timezoneList.length,tzi=0,tzv=null;if(0<tzl&&"object"!=typeof timezoneList[0])for(;tzi<tzl;tzi++)tzv=timezoneList[tzi],timezoneList[tzi]={value:tzv,label:$.timepicker.timezoneOffsetString(tzv,tp_inst.support.iso8601)};return tp_inst._defaults.timezoneList=timezoneList,tp_inst.timezone=null!==tp_inst._defaults.timezone?$.timepicker.timezoneOffsetNumber(tp_inst._defaults.timezone
):-1*(new Date).getTimezoneOffset(),tp_inst.hour=tp_inst._defaults.hour<tp_inst._defaults.hourMin?tp_inst._defaults.hourMin:tp_inst._defaults.hour>tp_inst._defaults.hourMax?tp_inst._defaults.hourMax:tp_inst._defaults.hour,tp_inst.minute=tp_inst._defaults.minute<tp_inst._defaults.minuteMin?tp_inst._defaults.minuteMin:tp_inst._defaults.minute>tp_inst._defaults.minuteMax?tp_inst._defaults.minuteMax:tp_inst._defaults.minute,tp_inst.second=tp_inst._defaults.second<tp_inst._defaults.secondMin?tp_inst._defaults.secondMin:tp_inst._defaults.second>tp_inst._defaults.secondMax?tp_inst._defaults.secondMax:tp_inst._defaults.second,
...
config.userActivityWidgets).render($(".user-activity",$container)),currentAssessmentActivity=currentAssessmentActivityFactory().render($(".assessment-activity",$container)),activityGraph=activityGraphFactory({autoRefresh:completedAssessmentsAutoRefreshInterval,autoRefreshBar:!0,graphConfig:{bindto:$(".js-completed-assessments",$container).selector,data:{url:url.route("completedAssessmentsData","Tools","taoProctoring")},axis:{x:{label:{text:__("Hours")}},y:{label:{text:__("Completed tests")},tick:{format:d3.format("d")}}},tooltip:{format:{name:function(){return __("Completed")}}},legend:{show:!1}}}).render(),$(".js-activity-chart-interval",$container).on(
"change",function(){var interval=$(this).val();activityGraph.refresh({graphConfig:{data:{url:url.route("completedAssessmentsData","Tools","taoProctoring",{interval:interval})},axis:{x:{tick:{format:"day"===interval?"%H:%M":"%m-%d"},label:{text:__("day"===interval?"Hours":"Days")}}}}})}),deliveriesList=deliveriesListFactory().render($(".js-delivery-list")),poll=polling({action:function(){request(url.route("assessmentActivityData","Tools","taoProctoring")).then(function(data){var uaData={};data&&data.group_user_activity&&_.forEach(data.group_user_activity,function(v,k){uaData[$.camelCase(k.replace(/_/g,"-"))]={value:v}}),userActivity.update(uaData),
currentAssessmentActivity.update({awaiting:{value:data&&data.awaiting_assessments},authorized:{value:data&&data.authorized_but_not_started_assessments},current:{value:data&&data.total_current_assessments},inProgress:{value:data&&data.in_progress_assessments},paused:{value:data&&data.paused_assessments}}),deliveriesList.update()}).catch(function(err){feedback().error(err.message),poll.stop()})}}).next().stop(),assessmentActivityAutoRefreshInterval&&(poll.setInterval(assessmentActivityAutoRefreshInterval),poll.start()),pauseReasonCategories=$(".js-pause-active-executions-container").data("reasoncategories"),$(".js-pause",$container).on("click",function(){
bulkActionPopup({renderTo:$(".js-pause-active-executions-container",$container),actionName:pauseMsg,reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(pauseReasonCategories.pause)}).on("ok",function(reason){!function(reason){request(url.route("pauseActiveExecutions","Tools","taoProctoring"),{reason:reason},"POST").then(function(data){helpers.loaded(),feedback().success(data.message)}).catch(function(err){helpers.loaded(),feedback().error(err.message)})}(reason)})})}}}),define("taoProctoring/helper/textConverter",["jquery","lodash","core/format","core/promise","util/url"],function($,_,format,Promise,url){"use strict"
;var pendingPromise,labels=null,proctoringLabels={get:function(message){null==labels[message]&&console.log(message);var localized=labels[message]?labels[message]:message;return 1<arguments.length&&(message=localized,localized=format.apply(null,arguments)),localized}};return function(){return pendingPromise||(pendingPromise=new Promise(function(resolve,reject){$.ajax({url:url.route("getAll","TextConverter","taoProctoring"),type:"POST",dataType:"json"}).done(function(data){labels=data.translations,resolve(proctoringLabels)}).fail(reject)})),pendingPromise}}),define("taoProctoring/loader/taoProctoring.bundle",function(){}),window.bundles=(window.bundles||[]
).concat(["taoQtiItem/loader/taoQtiItem.min","taoItems/loader/taoItems.min","taoQtiTest/loader/taoQtiTest.min"]);
