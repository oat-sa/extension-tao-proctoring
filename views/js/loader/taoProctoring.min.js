...
filterTransform:function(value){var first,last,dateFormat=locale.getDateTimeFormat().split(" ")[0],values=value.split(" "),result="";return 2<=values.length&&(first=values[0],last=values[values.length-1],result+=moment(first,dateFormat).format("X"),result+=" - ",result+=moment(last,dateFormat).add(1,"d").format("X")),result},customFilter:{template:'<input type="text" id="start_time_filter" name="filter[start_time]" placeholder="'+__("Filter")+'"/>',callback:function($elt){var lastValue,$filterContainer=$elt.closest(".filter"),dateFormatStr=locale.getDateTimeFormat().split(" ")[0],initialValue=!startDatePicker;$filterContainer.css("position","static"),
startDatePicker=dateTimePicker($filterContainer,{setup:"date-range",format:dateFormatStr,replaceField:$elt[0]}).on("ready",function(){initialValue&&(this.setValue([new Date,moment().add("1","d").toDate()]),$list.datatable("filter"))}).on("change",function(value){var selection=this.getSelectedDates();(""===value&&lastValue!==value||selection&&2===selection.length)&&$list.datatable("filter"),lastValue=value})}}}),model.push({id:"status",label:__("Status"),sortable:!0,filterable:!0,customFilter:{template:statusFilterTpl({statuses:_status.getStatuses()}),callback:statusFilterHandler},transform:function(value,row){var status,result=""
;return row&&row.state&&row.state.status&&(status=_status.getStatusByCode(row.state.status))&&(result=status.label,"INPROGRESS"===row.state.status&&(result=status.label),"Awaiting"===result&&highlightRows.push(row.id)),result}}),model.push({id:"authorizeCl",label:__("Authorize"),type:"actions",actions:[{id:"authorize",icon:"play",title:__("Authorize session"),disabled:function(){return!canDo("authorize",this.state)},action:authorize}]}),(null===data.canPause||data.canPause)&&model.push({id:"pauseCl",label:__("Pause"),type:"actions",actions:[{id:"pause",icon:"pause",title:__("Pause session"),disabled:function(){return!canDo("pause",this.state)},action:pause}]
...
taskCreationButton.getElement().css({float:"right"}))}}}),define("taoProctoring/component/history/historyTable",["jquery","lodash","i18n","ui/component","ui/datatable"],function($,_,__,component){"use strict";var _defaults={sortBy:"timestamp",sortOrder:"desc"};function renderDetails(details){return _.isArray(details)&&(details=details.join("<br />")),details}return function(config,data){var initConfig=_.defaults(config||{},_defaults);return component({refresh:function(params){var $element;this.is("rendered")?($element=this.getElement(),params&&$element.datatable("options",{params:params}),$element.datatable("refresh")):params&&(initConfig.params=_.merge(
initConfig.params,params))}}).on("render",function(){var self=this,tools=[{id:"refresh",icon:"reset",title:__("Refresh the page"),label:__("Refresh"),action:function(){self.refresh()}}];this.getElement().on("query.datatable",function(){self.trigger("loading")}).on("load.datatable",function(){self.trigger("loaded")}).on("error.datatable",function(e,err){self.trigger("error",err)}).datatable({url:initConfig.service,params:initConfig.params,sortby:initConfig.sortBy,sortorder:initConfig.sortOrder,status:{empty:__("No history to display!"),available:__("Available history"),loading:__("Loading")},selectable:!(!initConfig.tools||!_.find(initConfig.tools,{
massAction:!0})),tools:tools.concat(initConfig.tools||[]),model:[{id:"date",label:__("Date"),sortable:!0},{id:"role",label:__("Role"),sortable:!0},{id:"actor",label:__("Actor"),sortable:!0},{id:"event",label:__("Event"),sortable:!0},{id:"details",label:__("Details"),transform:renderDetails},{id:"context",label:__("Context"),transform:renderDetails}]},data)}).init(initConfig)}}),define("tpl!taoProctoring/templates/reporting/index",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,buffer="",
