...
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="icon-pause"></span><span class="number">',(helper=helpers.paused)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.paused,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n</p>"})}),define("taoProctoring/controller/Delivery/index",["lodash","jquery","i18n","helpers","layout/loading-bar","ui/listbox","util/encode","ui/feedback","ui/bulkActionPopup","ui/cascadingComboBox","taoProctoring/helper/status","taoProctoring/component/breadcrumbs","tpl!taoProctoring/templates/delivery/listBoxActions","tpl!taoProctoring/templates/delivery/listBoxStats"],function(_,$,__,helpers,loadingBar,listBoxFactory,encode,feedback,bulkActionPopup,cascadingComboBox,_status,breadcrumbsFactory,actionsTpl,statsTpl){
"use strict";var cssScope=".delivery-index",taoProctoringCtlr={start:function(){function format(data){return _.transform(data,function(result,delivery){var props=delivery.properties,tplData={locked:delivery.stats.awaitingApproval,inProgress:delivery.stats.inProgress,paused:delivery.stats.paused};props&&props.periodStart&&props.periodEnd&&(tplData.showProperties=!0,tplData.periodStart=props.periodStart,tplData.periodEnd=props.periodEnd,delivery.cls="has-properties-displayed"),delivery.html=actionsTpl({id:delivery.id}),delivery.content=statsTpl(tplData),result[delivery.id]=delivery},{})}function getDeliveryLabel(id){return deliveries[id]&&deliveries[id].label}function update(data){
deliveries=format(data),listBox.update(data),loadingBar.stop()}function refresh(){loadingBar.start(),listBox.setLoading(!0),$.ajax({url:serviceUrl,cache:!1,dataType:"json",type:"GET"}).done(function(data){update(data)})}function pause(deliveryId,selection,deliveryExecutions){var allowed=_.map(selection,function(data){return{id:data.id,label:data.testTaker.firstName+" "+data.testTaker.lastName}});bulkActionPopup({renderTo:$container,actionName:__("Pause Session"),reason:!0,resourceType:"test taker",categoriesSelector:cascadingComboBox({categoriesDefinitions:categories.pause.categoriesDefinitions,categories:categories.pause.categories}),allowedResources:allowed
}).on("ok",function(reason){$.ajax({url:helpers._url("pauseExecutions","Delivery","taoProctoring"),data:{delivery:deliveryId,testCenter:testCenterId,execution:_.pluck(selection,"id"),reason:reason},dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){var messageContext,unprocessed;loadingBar.stop(),response&&response.success?(feedback().success("Selected deliveries successfully paused"),refresh()):(messageContext="",response&&(unprocessed={},_.forEach(response.unprocessed,function(id){var execution=deliveryExecutions[id],uri=execution&&execution.delivery&&execution.delivery.uri;uri&&(unprocessed[uri]=(unprocessed[uri]||0)+1);
}),unprocessed=_.map(unprocessed,function(count,uri){return count>1?__("%d sessions of the delivery %s have not been paused",count,getDeliveryLabel(uri)):__("A session of the delivery %s have not been paused",getDeliveryLabel(uri))}),unprocessed.length&&(messageContext+="<br>"+unprocessed.join("<br>")),response.error&&(messageContext+="<br>"+encode.html(response.error))),feedback().warning(__("Something went wrong ...")+"<br>"+messageContext,{encodeHtml:!1}))})})}var $container=$(cssScope),list=$container.data("list"),deliveries=format(list),crumbs=$container.data("breadcrumbs"),categories=$container.data("categories"),testCenterId=$container.data("testcenter"),serviceUrl=helpers._url("deliveries","Delivery","taoProctoring",{
testCenter:testCenterId}),listBox=listBoxFactory({title:__("Sessions"),textEmpty:__("No sessions available"),textNumber:__("Available"),textLoading:__("Loading"),renderTo:$container.find(".content"),replace:!0,list:list,width:12,countRenderer:function(count){return count-1}});breadcrumbsFactory($container,crumbs),$container.on("click",".pause",function(e){var deliveryId=$(this).data("delivery"),pauseUrl="all"===deliveryId?helpers._url("allDeliveriesExecutions","Delivery","taoProctoring",{testCenter:testCenterId}):helpers._url("deliveryExecutions","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId});e.stopPropagation(),e.preventDefault(),$.get(pauseUrl,function(res){
var inProgressExecs,deliveryExecutions={};_.isPlainObject(res)&&_.isArray(res.data)&&(inProgressExecs=_.filter(res.data,function(data){return deliveryExecutions[data.id]=data,data.state&&data.state.status===_status.getStatus("inprogress").code}),inProgressExecs.length?pause(deliveryId,inProgressExecs,deliveryExecutions):feedback().info(__("There is no delivery in progress")))})}),list?loadingBar.stop():refresh()}};return loadingBar.start(),taoProctoringCtlr}),define("taoProctoring/controller/Delivery/manage",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","ui/dialog","taoProctoring/component/breadcrumbs","ui/datatable"],function($,__,helpers,loadingBar,encode,feedback,dialog,breadcrumbsFactory){
...
define("tpl!taoProctoring/templates/delivery/approximatedTimer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n<span class="approximated-timer" ',stack1=helpers.if.call(depth0,depth0&&depth0.since,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+=">~",(helper=helpers.timer)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.timer,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n",
stack1=helpers.if.call(depth0,depth0&&depth0.since,{hash:{},inverse:self.noop,fn:self.program(4,program4,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n"}function program2(depth0,data){var stack1,helper,buffer="";return buffer+=' title="',(helper=helpers.since)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.since,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"'}function program4(depth0,data){var stack1,helper,buffer="";return buffer+='<span class="icon-warning" title="',(helper=helpers.since)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.since,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></span>'}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return stack1=helpers.if.call(depth0,depth0&&depth0.timer,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n"})}),define("taoProctoring/controller/Delivery/monitoring",["jquery","lodash","i18n","helpers","moment","layout/loading-bar","core/encoder/time","util/encode","ui/feedback","ui/dialog","ui/bulkActionPopup","ui/cascadingComboBox","taoProctoring/component/extraTime/extraTime","taoProctoring/component/extraTime/encoder","taoProctoring/component/breadcrumbs","taoProctoring/helper/status","tpl!taoProctoring/templates/delivery/deliveryLink","tpl!taoProctoring/templates/delivery/statusFilter","tpl!taoProctoring/templates/delivery/approximatedTimer","ui/datatable","select2"],function($,_,__,helpers,moment,loadingBar,timeEncoder,encode,feedback,dialog,bulkActionPopup,cascadingComboBox,extraTimePopup,encodeExtraTime,breadcrumbsFactory,_status,deliveryLinkTpl,statusFilterTpl,approximatedTimerTpl){
"use strict";var cssScope=".delivery-monitoring",extraTimeUnit=60;return loadingBar.start(),{start:function(){function request(url,selection,data,message){selection&&selection.length&&(loadingBar.start(),$.ajax({url:url,data:_.merge({execution:selection},data),dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){var messageContext,unprocessed;loadingBar.stop(),response&&response.success?(message&&feedback().success(message),$list.datatable("refresh")):(messageContext="",response&&(unprocessed=_.map(response.unprocessed,function(id){var execution=getExecutionData(id);if(execution)return __("Session %s - %s has not been processed",execution.delivery,execution.date);
}),unprocessed.length&&(messageContext+="<br>"+unprocessed.join("<br>")),response.error&&(messageContext+="<br>"+encode.html(response.error))),feedback().error(__("Something went wrong ...")+"<br>"+messageContext,{encodeHtml:!1}))}))}function authorise(selection){execBulkAction("authorize",__("Authorize Session"),selection,function(sel,reason){request(authoriseUrl,sel,{reason:reason},__("Sessions authorized"))})}function pause(selection){execBulkAction("pause",__("Pause Session"),selection,function(sel,reason){request(pauseUrl,sel,{reason:reason},__("Sessions paused"))})}function terminate(selection){execBulkAction("terminate",__("Terminate Session"),selection,function(sel,reason){
request(terminateUrl,sel,{reason:reason},__("Sessions terminated"))})}function report(selection){execBulkAction("report",__("Report Irregularity"),selection,function(sel,reason){request(reportUrl,sel,{reason:reason},__("Sessions reported"))})}function print(selection,type){execBulkAction("print",__("Print Score"),selection,function(sel){window.open(helpers._url(type,"Reporting","taoProctoring",{id:sel}),"printReport"+JSON.stringify(sel))})}function terminateAndIrregularity(selection){dialog({message:__("Please, make your selection"),autoRender:!0,autoDestroy:!0,buttons:[{id:"terminate",type:"error",label:__("Terminate session"),icon:"stop",close:!0,action:function(){
terminate(selection)}},{id:"irregularity",type:"info",label:__("Report irregularity"),icon:"delivery-small",close:!0,action:function(){report(selection)}}]})}function showHistory(selection){var urlParams={testCenter:testCenterId,session:selection};deliveryId&&(urlParams.delivery=deliveryId),window.location.href=helpers._url("sessionHistory","Reporting","taoProctoring",urlParams)}function printReport(selection){print(selection,"printReport")}function printResults(selection){print(selection,"printRubric")}function timeHandling(selection){var _selection=_.isArray(selection)?selection:[selection],config=_.merge(listSessions("time",_selection),{renderTo:$content,actionName:__("Grant Extra Time"),
unit:extraTimeUnit});extraTimePopup(config).on("ok",function(time){request(extraTimeUrl,_selection,{time:time},__("Extra time granted"))})}function canDo(what,state){var status;return!(!state||!state.status)&&(status=_status.getStatusByCode(state.status),status&&status.can[what]===!0)}function verifyDelivery(testTakerData,actionName){var deliveryName,formatted,status;return deliveryName=_.isObject(testTakerData.delivery)?testTakerData.delivery.label:$(testTakerData.delivery).text(),formatted={id:testTakerData.id,label:deliveryName+" ["+testTakerData.date+"] "+testTakerData.firstname+" "+testTakerData.lastname},status=_status.getStatusByCode(testTakerData.state.status),
status&&(formatted.allowed=status.can[actionName]===!0,formatted.allowed||(formatted.reason=status.can[actionName])),testTakerData.timer&&(formatted.extraTime=testTakerData.timer.extraTime,formatted.consumedTime=testTakerData.timer.consumedExtraTime,formatted.remaining=testTakerData.timer.remaining),formatted}function getExecutionData(uri){return _.find(dataset.data,{id:uri})}function listSessions(actionName,selection){var allowedDeliveries=[],forbiddenDeliveries=[];return _.each(selection,function(uri){var checkedDelivery,testTakerData=getExecutionData(uri);testTakerData&&(checkedDelivery=verifyDelivery(testTakerData,actionName),checkedDelivery.allowed?allowedDeliveries.push(checkedDelivery):forbiddenDeliveries.push(checkedDelivery));
}),{resourceType:"session",allowedResources:allowedDeliveries,deniedResources:forbiddenDeliveries}}function execBulkAction(actionName,actionTitle,selection,cb){var config,_selection=_.isArray(selection)?selection:[selection],askForReason=categories[actionName]&&categories[actionName].categoriesDefinitions&&categories[actionName].categoriesDefinitions.length;config=_.merge(listSessions(actionName,_selection),{renderTo:$content,actionName:actionTitle,reason:askForReason,reasonRequired:!0,categoriesSelector:cascadingComboBox(categories[actionName]||{})}),config.allowedResources.length?bulkActionPopup(config).on("ok",function(reason){_.isFunction(cb)&&cb(_selection,reason);
}):_selection.length>1?feedback().warning(__("No report available for these test sessions")):feedback().warning(__("No report available for this test session"))}function buildStatusFilter(){return statusFilterTpl({statuses:_status.getStatuses()})}function statusFilterHandler($el){$el.select2({dropdownAutoWidth:!0,placeholder:__("Filter"),minimumResultsForSearch:1/0,allowClear:!0})}var actionButtons,actionList,$container=$(cssScope),$content=$container.find(".content"),$list=$container.find(".list"),crumbs=$container.data("breadcrumbs"),dataset=$container.data("set"),extraFields=$container.data("extrafields"),categories=$container.data("categories"),deliveryId=$container.data("delivery"),isManageable=$container.data("ismanageable"),testCenterId=$container.data("testcenter"),approximateTimer=$container.data("approximatetimer"),approximateTimerWarning=$container.data("approximatetimerwarning"),timeHandlingButton=$container.data("timehandling"),printReportButton=$container.data("printreportbutton"),refreshBtn=$container.data("refreshbtn"),autoRefresh=$container.data("autorefresh"),manageUrl=helpers._url("manage","Delivery","taoProctoring",{
delivery:deliveryId,testCenter:testCenterId}),terminateUrl=helpers._url("terminateExecutions","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),pauseUrl=helpers._url("pauseExecutions","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),authoriseUrl=helpers._url("authoriseExecutions","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),extraTimeUrl=helpers._url("extraTime","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),reportUrl=helpers._url("reportExecutions","Delivery","taoProctoring",{delivery:deliveryId,testCenter:testCenterId}),serviceUrl=helpers._url("deliveryExecutions","Delivery","taoProctoring",{
delivery:deliveryId,testCenter:testCenterId}),serviceAllUrl=helpers._url("allDeliveriesExecutions","Delivery","taoProctoring",{testCenter:testCenterId}),tools=[],model=[],highlightRows=[];breadcrumbsFactory($container,crumbs),refreshBtn&&tools.push({id:"refresh",icon:"reset",title:__("Refresh the page"),label:__("Refresh"),action:function(){$list.datatable("refresh")}}),autoRefresh&&setInterval(function(){$list.datatable("refresh")},autoRefresh),deliveryId&&isManageable&&tools.push({id:"manage",icon:"property-advanced",title:__("Manage sessions"),label:__("Manage"),action:function(){window.location.href=manageUrl}}),tools.push({id:"authorise",icon:"play",title:__("Authorize sessions"),
...
massAction:!0,action:printResults}),printReportButton&&tools.push({id:"printReport",title:__("Print the assessment results"),icon:"result",label:__("Print Results"),massAction:!0,action:printReport}),timeHandlingButton&&tools.push({id:"timeHandling",title:__("Session time handling"),icon:"time",label:__("Time"),massAction:!0,action:timeHandling}),deliveryId||model.push({id:"delivery",label:__("Session"),sortable:!0,transform:function(value,row){var delivery=row&&row.delivery;return delivery&&(delivery.url=helpers._url("monitoring","Delivery","taoProctoring",{delivery:delivery.uri,testCenter:testCenterId}),value=deliveryLinkTpl(delivery)),value}}),model.push({
id:"firstname",label:__("First name"),sortable:!0,transform:function(value,row){return row&&row.testTaker&&row.testTaker.firstName||""}}),model.push({id:"lastname",label:__("Last name"),sortable:!0,transform:function(value,row){return row&&row.testTaker&&row.testTaker.lastName||""}}),_.each(extraFields,function(extraField){model.push({id:extraField.id,label:extraField.label,sortable:!0,transform:function(value,row){return row&&row.extraFields&&row.extraFields[extraField.id]||""}})}),model.push({id:"date",sortable:!0,label:__("Started at")}),model.push({id:"status",label:__("Status"),sortable:!0,filterable:!0,customFilter:{template:buildStatusFilter(),callback:statusFilterHandler
},transform:function(value,row){var status,result="";return row&&row.state&&row.state.status&&(status=_status.getStatusByCode(row.state.status),status&&(result=status.label,"INPROGRESS"===row.state.status&&(result=status.label),"Awaiting"===result&&highlightRows.push(row.id))),result}}),model.push({id:"authorizeCl",label:__("Authorize"),type:"actions",actions:[{id:"authorise",icon:"play",title:__("Authorize session"),disabled:function(){return!canDo("authorize",this.state)},action:authorise}]}),model.push({id:"pauseCl",label:__("Pause"),type:"actions",actions:[{id:"pause",icon:"pause",title:__("Pause session"),disabled:function(){return!canDo("pause",this.state);
},action:pause}]}),model.push({id:"remaining",sortable:!0,label:__("Remaining"),transform:function(value,row){var timer=_.isObject(row.timer)?row.timer:{},refinedValue=approximateTimer?timer.approximatedRemaining:timer.remaining,remaining=parseInt(refinedValue,10),since=parseInt(timer.since,10);return(remaining||_.isFinite(remaining))&&(remaining<0&&(timer.extraTime&&(timer.consumedExtraTime+=-remaining),remaining=0),refinedValue=remaining?timeEncoder.encode(remaining):"",!remaining&&!timer.extraTime||timer.extraTime&&timer.extraTime<=timer.consumedExtraTime?refinedValue=__("Timed out"):refinedValue+=encodeExtraTime(timer.extraTime,timer.consumedExtraTime,__("%s min"),extraTimeUnit),
approximateTimer&&(refinedValue=approximatedTimerTpl({timer:refinedValue,since:approximateTimerWarning&&_.isFinite(since)&&since>=approximateTimerWarning?__("Last test taker's activity: %s",moment.duration(-since,"seconds").humanize(!0)):null}))),refinedValue}}),timeHandlingButton&&model.push({id:"extraTime",label:__("Extra Time"),type:"actions",actions:[{id:"timeHandling",title:__("Session time handling"),icon:"time",action:timeHandling,hidden:function(){return!canDo("time",this.state)}}]}),model.push({id:"connectivity",sortable:!0,label:__("Connectivity"),transform:function(value,row){return row.state.status===_status.STATUS_INPROGRESS?__(row.online?"online":"offline"):"";
...
return status.feedback.message}}),model.push({id:"bandwidth",label:__("Bandwidth"),transform:function(value){var bandwidth=formatNumber(value);return value>100&&(bandwidth="> 100"),bandwidth+" Mbs"}}),model.push({id:"date",label:__("Date")}),$list.on("query.datatable",function(){loadingBar.start()}).on("load.datatable",function(){loadingBar.stop()}).datatable({url:serviceUrl,status:{empty:__("No readiness checks have been done!"),available:__("Readiness checks already done"),loading:__("Loading")},tools:tools,actions:actions,selectable:!0,model:model},dataset)}};return taoProctoringDiagnosticCtlr}),define("taoProctoring/controller/Irregularity/index",["lodash","jquery","i18n","ui/feedback","layout/loading-bar","helpers","jquery.fileDownload"],function(_,$,__,feedback,loadingBar,helpers){
"use strict";var IrregularityCtlr={start:function(){var $form=$("#export-form");$form.on("submit",function(e){e.stopPropagation(),e.preventDefault(),loadingBar.start();var self=$(this),uri=$('[name="uri"]',self).val(),from=$('[name="from"]',self).val(),to=$('[name="to"]',self).val(),params={uri:uri,from:from,to:to},exportUrl=helpers._url("exportIrregularities","Irregularity","taoProctoring",params);$.fileDownload(exportUrl,{successCallback:function(){loadingBar.stop()},failCallback:function(jqXHR){loadingBar.stop();var response=$.parseJSON($(jqXHR).text());response&&feedback().error(new Error(response.message))}})})}};return IrregularityCtlr}),define("tpl!taoProctoring/component/proctorForm/form",["handlebars"],function(hb){
return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function";return buffer+='<div class="proctor-form section-container">\n    <div class="content-block">\n        ',(helper=helpers.form)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.form,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n    </div>\n</div>"})}),define("taoProctoring/component/proctorForm",["lodash","jquery","i18n","helpers","users","ui/feedback","ui/component","taoProctoring/helper/textConverter","tpl!taoProctoring/component/proctorForm/form"],function(_,$,__,helpers,users,feedback,component,textConverter,formTpl){
"use strict";function renderFormFromData($container,formData){$container.html(formTpl({form:formData.form})),users.checkLogin(formData.loginId,proctorLoginCheckUrl)}var _ns=".proctor-form",proctorFormUrl=helpers._url("createProctorForm","ProctorManager","taoProctoring"),proctorLoginCheckUrl=helpers._url("checkLogin","ProctorManager","taoProctoring");return helpers.init(),function(config){return component().on("destroy",function(){this.getElement().off(_ns).empty()}).on("render",function(){var self=this,$element=this.getElement();textConverter().then(function(labels){$.get(proctorFormUrl,function(formData){renderFormFromData($element,formData),$element.on("submit"+_ns,"form",function(e){
var $form=$(this),fields=$form.serializeArray(),list=self.config.testCenterList,data={testCenters:list&&list.getSelection()};_.each(fields,function(field){data[field.name]=field.value}),$.post(proctorFormUrl,data,function(res){res.created?(feedback().success(labels.get("Proctor created")),self.destroy()):renderFormFromData($element,res)}),e.preventDefault()}).on("click","button.btn-diasble",function(){self.destroy()})})}).catch(function(err){console.log(err)})}).init(config)}}),define("tpl!taoProctoring/templates/proctorManager/counters",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){
...
}};return component(historyTable).on("render",function(){var self=this,tools=[{id:"refresh",icon:"reset",title:__("Refresh the page"),label:__("Refresh"),action:function(){self.refresh()}}];this.getElement().on("query.datatable",function(){self.trigger("loading")}).on("load.datatable",function(){self.trigger("loaded")}).datatable({url:initConfig.service,params:initConfig.params,sortby:initConfig.sortBy,sortorder:initConfig.sortOrder,status:{empty:__("No history to display!"),available:__("Available history"),loading:__("Loading")},selectable:!(!initConfig.tools||!_.find(initConfig.tools,{massAction:!0})),tools:tools.concat(initConfig.tools||[]),model:[{id:"date",
label:__("Date"),sortable:!0},{id:"role",label:__("Role"),sortable:!0},{id:"actor",label:__("Actor"),sortable:!0},{id:"event",label:__("Event"),sortable:!0},{id:"details",label:__("Details"),transform:renderDetails},{id:"context",label:__("Context"),transform:renderDetails}]},data)}).init(initConfig)}var _defaults={sortBy:"timestamp",sortOrder:"desc"};return historyTableFactory}),define("taoProctoring/controller/Reporting/history",["jquery","lodash","i18n","helpers","layout/loading-bar","util/encode","taoProctoring/component/dateRange","taoProctoring/component/history/historyTable","taoProctoring/component/breadcrumbs","ui/datatable"],function($,_,__,helpers,loadingBar,encode,dateRangeFactory,historyTableFactory,breadcrumbsFactory){
"use strict";var cssScope=".session-history",taoProctoringReportCtlr={start:function(){var $container=$(cssScope),dataset=$container.data("set"),testCenterId=$container.data("testcenter"),deliveryId=$container.data("delivery"),sessions=$container.data("sessions"),sortBy=$container.data("sortBy"),sortOrder=$container.data("sortorder"),periodStart=$container.data("periodstart"),periodEnd=$container.data("periodEnd"),serviceUrl=helpers._url("history","Reporting","taoProctoring",{testCenter:testCenterId,delivery:deliveryId,session:sessions}),monitoringUrl=helpers._url("monitoring","Delivery","taoProctoring",{testCenter:testCenterId,delivery:deliveryId}),monitoringAllUrl=helpers._url("monitoringAll","Delivery","taoProctoring",{
testCenter:testCenterId}),detailedHistory=!1,historyTable=historyTableFactory({tools:[{id:"back",icon:"preview",title:__("Return to the session monitoring"),label:__("Monitoring"),action:function(){window.location.href=deliveryId?monitoringUrl:monitoringAllUrl}},{id:"show-detailed-report",icon:"insert-horizontal-line",title:__("Show detailed session history messages"),label:__("Show detailed report"),action:function(){var tool=_.find(historyTable.config.tools,{id:"show-detailed-report"});historyTable.config.params.detailed=detailedHistory=!detailedHistory,tool.label=__(detailedHistory?"Show brief report":"Show detailed report"),historyTable.refresh()}}],params:{
detailed:detailedHistory},service:serviceUrl,sortBy:sortBy,sortOrder:sortOrder},dataset).on("loading",function(){loadingBar.start()}).on("loaded",function(){loadingBar.stop()}).render($container.find(".list"));breadcrumbsFactory($container,$container.data("breadcrumbs")),dateRangeFactory({start:periodStart,end:periodEnd,renderTo:$container.find(".panel")}).on("change submit",function(){historyTable.refresh({periodStart:this.getStart(),periodEnd:this.getEnd()})})}};return loadingBar.start(),taoProctoringReportCtlr}),define("tpl!taoProctoring/templates/testSiteAdmin/adminLink",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<a href="',(helper=helpers.href)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.href,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="admin-link">\n    <span class="icon-users"></span>\n    ',(helper=helpers.manageProctorMenu)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.manageProctorMenu,stack1=typeof helper===functionType?helper.call(depth0,{
hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"\n</a>"})}),define("taoProctoring/controller/TestCenter/index",["jquery","i18n","helpers","layout/loading-bar","ui/listbox","taoProctoring/component/breadcrumbs","taoProctoring/helper/textConverter","tpl!taoProctoring/templates/testSiteAdmin/adminLink"],function($,__,helpers,loadingBar,listBox,breadcrumbsFactory,textConverter,adminLinkTpl){"use strict";var cssScope=".testcenters-index";loadingBar.start();var taoProctoringIndexCtlr={start:function(){textConverter().then(function(labels){var $container=$(cssScope),admin=$container.data("administrator"),boxes=$container.data("list"),crumbs=$container.data("breadcrumbs"),list=listBox({
...
},api={add:function($container,options){return this.on("render",function(){var deliveryTree,testTakerTree,self=this;options=_.defaults(options||{},this.config),deliveryTree=buildDeliveryTree(deliveryTreeId,options.dataUrl),testTakerTree=buildTestTakerTree(testTakerTreeId,options.dataUrl),initModal({width:650}),this.$component.on("click"+_ns,".actions .done",function(e){var deliveries=_(deliveryTree.getChecked()).uniq().compact().value(),testTakers=_(testTakerTree.getChecked()).uniq().compact().value();deliveries&&deliveries.length?(self.trigger("ok",{deliveries:deliveries,testTakers:testTakers}),destroyModal()):feedback(self.$component).warning(__("At least one delivery need to be selected to create"));
}).on("click"+_ns,".actions .cancel",function(e){e.preventDefault(),destroyModal(),self.trigger("cancel")}),this.trigger("open")}).init({title:__("Add Eligibility"),editingMode:!1,subjectTreeId:testTakerTreeId,deliveryTreeId:deliveryTreeId}).render($container)},edit:function($container,deliveryName,testTakers,options){return this.on("render",function(){var testTakerTree,self=this;options=_.defaults(options||{},this.config),testTakerTree=buildTestTakerTree(testTakerTreeId,options.dataUrl,testTakers),initModal({width:400}),this.$component.on("click"+_ns,".actions .done",function(e){var testTakers=_(testTakerTree.getChecked()).uniq().compact().value();self.trigger("ok",{
testTakers:testTakers}),destroyModal()}).on("click"+_ns,".actions .cancel",function(e){e.preventDefault(),destroyModal(),self.trigger("cancel")}),this.trigger("open")}).init({title:__("Edit Eligibility"),editingMode:!0,subjectTreeId:testTakerTreeId,deliveryTreeId:deliveryTreeId,deliveryName:deliveryName}).render($container)}};return eligibilityEditor=component(api,config).setTemplate(layoutTpl)};return eligibilityEditorFactory}),define("tpl!taoProctoring/component/eligibilityTable/status",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var helper,options,buffer="";return buffer+='\n    <span class="icon-unshield txt-danger" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"This delivery can be launched without authorization",options):helperMissing.call(depth0,"__","This delivery can be launched without authorization",options)))+'"></span>\n'}function program3(depth0,data){var helper,options,buffer="";return buffer+='\n    <span class="icon-shield txt-success" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"This delivery needs proctor authorization",options):helperMissing.call(depth0,"__","This delivery needs proctor authorization",options)))+'"></span>\n'}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
