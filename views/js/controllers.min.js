...
},action:pause}]}),model.push({id:"remaining",sortable:!0,label:__("Remaining"),transform:function(value,row){var timer=_.isObject(row.timer)?row.timer:{},refinedValue=approximateTimer?timer.approximatedRemaining:timer.remaining,remaining=parseInt(refinedValue,10),since=parseInt(timer.since,10);return(remaining||_.isFinite(remaining))&&(remaining<0&&timer.extraTime&&(timer.consumedExtraTime+=-remaining,remaining=0),refinedValue=remaining?timeEncoder.encode(remaining):"",!remaining||timer.extraTime&&timer.extraTime<=timer.consumedExtraTime?refinedValue=__("Timed out"):refinedValue+=encodeExtraTime(timer.extraTime,timer.consumedExtraTime,__("%s min"),extraTimeUnit),
