...
})}),define("taoProctoring/controller/TaoProctoring/index",["jquery","helpers","layout/loading-bar","tpl!taoProctoring/tpl/entryPoint"],function($,helpers,loadingBar,entryPointTpl){var refreshPolling=6e4,cssScope=".deliveries-listing",hiddenCls="hidden";loadingBar.start();var taoProctoringCtlr={start:function(){var $titleLoading=$(cssScope+" .loading"),$titleEmpty=$(cssScope+" .empty-list"),$titleAvailable=$(cssScope+" .available-list"),$titleCount=$(cssScope+" .count"),$list=$(cssScope+" .list"),listEntries=$list.data("list"),serviceUrl=helpers._url("deliveries","TaoProctoring","taoProctoring"),pollTo=null,update=function(entries){pollTo&&(clearTimeout(pollTo),pollTo=null),$titleLoading.addClass(hiddenCls),$list.empty(),entries&&entries.length?($titleAvailable.removeClass(hiddenCls),$list.append(entryPointTpl({entries:entries})),$titleCount.text(entries.length),$titleEmpty.addClass(hiddenCls)):($titleEmpty.removeClass(hiddenCls),$titleAvailable.addClass(hiddenCls)),loadingBar.stop(),refreshPolling&&(pollTo=setTimeout(refresh,refreshPolling))
