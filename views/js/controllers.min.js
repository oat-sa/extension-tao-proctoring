...
transform:renderDetails}]},data)}).init(initConfig)}var _defaults={sortBy:"timestamp",sortOrder:"desc"};return historyTableFactory}),define("taoProctoring/controller/Reporting/history",["jquery","lodash","i18n","helpers","layout/loading-bar","util/encode","taoProctoring/component/dateRange","taoProctoring/component/history/historyTable","taoProctoring/component/breadcrumbs","ui/datatable"],function($,_,__,helpers,loadingBar,encode,dateRangeFactory,historyTableFactory,breadcrumbsFactory){"use strict";var cssScope=".session-history",taoProctoringReportCtlr={start:function(){var $container=$(cssScope),dataset=$container.data("set"),deliveryId=($container.data("testcenter"),
$container.data("delivery")),sessions=$container.data("sessions"),sortBy=$container.data("sortBy"),sortOrder=$container.data("sortorder"),periodStart=$container.data("periodstart"),periodEnd=$container.data("periodEnd"),serviceUrl=helpers._url("history","Reporting","taoProctoring",{delivery:deliveryId,session:sessions}),detailedHistory=!1,historyTable=historyTableFactory({tools:[{id:"show-detailed-report",icon:"insert-horizontal-line",title:__("Show detailed session history messages"),label:__("Show detailed report"),action:function(){var tool=historyTable.config.tools.find(function(val){return"show-detailed-report"===val.id});historyTable.config.params.detailed=detailedHistory=!detailedHistory,
tool.label=__(detailedHistory?"Show brief report":"Show detailed report"),historyTable.refresh()}}],params:{detailed:detailedHistory},service:serviceUrl,sortBy:sortBy,sortOrder:sortOrder},dataset).on("loading",function(){loadingBar.start()}).on("loaded",function(){loadingBar.stop()}).render($container.find(".list"));breadcrumbsFactory($container,$container.data("breadcrumbs")),dateRangeFactory({start:periodStart,end:periodEnd,renderTo:$container.find(".panel")}).on("change submit",function(){historyTable.refresh({periodStart:this.getStart(),periodEnd:this.getEnd()})})}};return loadingBar.start(),taoProctoringReportCtlr}),define("taoProctoring/controller/Tools/pauseActiveExecutions",["jquery","i18n","helpers","util/url","ui/feedback","ui/cascadingComboBox","ui/bulkActionPopup"],function($,__,helpers,url,feedback,cascadingComboBox,bulkActionPopup){
"use strict";function doPause(reason){$.ajax({type:"POST",data:{reason:reason},url:url.route("pauseActiveExecutions","Tools","taoProctoring"),dataType:"json",success:function(data){helpers.loaded(),data.success?feedback().success(data.message):feedback().error(data.message)}})}var $container=$(".js-pause-active-executions-container"),categories=$container.data("reasoncategories"),msg=__("Warning, you are about to pause all in progress tests. All test takers will be paused on or before the next heartbeat. Please provide a reason for this action.");return{start:function(){$(".js-pause").on("click",function(){var config;config={renderTo:$container,actionName:msg,
reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(categories.pause)},bulkActionPopup(config).on("ok",function(reason){doPause(reason)})})}}});
