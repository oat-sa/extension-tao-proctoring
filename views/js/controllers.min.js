define("taoProctoring/controller/routes",{Reporting:{actions:{sessionHistory:"controller/Reporting/history"}},Irregularity:{actions:{index:"controller/Irregularity/index"}},DeliverySelection:{actions:{index:"controller/Delivery/index"}},Monitor:{actions:{index:"controller/Delivery/monitoring"}},Tools:{actions:{pauseActiveExecutions:"controller/Tools/pauseActiveExecutions"}}}),define("taoProctoring/controller/app",["jquery","core/historyRouter","core/logger","core/eventifier","core/statifier","util/url","ui/feedback"],function($,historyRouterFactory,loggerFactory,eventifier,statifier,urlUtil,feedback){"use strict";var historyRouter,appLogger=loggerFactory("application"),appController=eventifier(statifier({
start:function(){appController.apply(),appController.forward(window.location+"")},apply:function(selector,target){selector=selector||".router",target=target||document,$(target).off("click.appController").on("click.appController",selector,function(e){var $elt,href;e.preventDefault(),$elt=$(this),href=$elt.attr("href"),href||(href=$("[href]:first-child",$elt).attr("href")),href&&appController.redirect(href)})},redirect:function(url){historyRouter.trigger("dispatch",url)},forward:function(url){historyRouter.dispatch({url:url},!0)},getRouter:function(){return historyRouter},getLogger:function(){return appLogger},onError:function(err){var message=err&&err.message||err;
appLogger.error(err),feedback().error(message)}}));return historyRouter=historyRouterFactory().on("dispatching",function(url){appController.setState("dispatching"),appController.trigger("change",url)}).on("dispatched",function(url){appController.setState("dispatching",!1),appController.setState("ready"),appController.trigger("started",url)}),appController}),define("taoProctoring/helper/status",["lodash","i18n"],function(_,__){"use strict";function getStatus(statusName){return _status[statusName]}function getStatusByCode(statusCode){return _.find(_status,{code:statusCode})}function verifyTestTaker(testTakerData,actionName){var formatted={id:testTakerData.id,label:testTakerData.firstname+" "+testTakerData.lastname
...
}):_selection.length>1?feedback().warning(__("No report available for these test sessions")):feedback().warning(__("No report available for this test session"))}function buildStatusFilter(){return statusFilterTpl({statuses:_status.getStatuses()})}function setTagUsage(applyTags){if(defaultTag){if(!$list.find(".tag").length){var $filter=$('<span class="filter"><input type="hidden" name="tag" class="tag" value="'+applyTags+'"/></span>');$filter.appendTo($list)}$list.find(".tag").val(applyTags),$list.data("applytags",applyTags),applyTags?($list.find(".action-bar").children(".tool-tag").hide(),tagWaringBlock=feedback().warning(__('Currently you are only viewing the test session in the "%s" group',defaultTag),{
timeout:{success:-1}})):($list.find(".action-bar").children(".tool-notag").hide(),tagWaringBlock.close())}}function setInitialFilters(){var now=new Date,nowStr=now.getFullYear()+"/"+("0"+(now.getMonth()+1)).slice(-2)+"/"+("0"+now.getDate()).slice(-2);$("#start_time_filter").val(nowStr+" - "+nowStr),defaultTag&&setTagUsage(!0),$list.datatable("filter")}function statusFilterHandler($el){$el.select2({dropdownAutoWidth:!0,placeholder:__("Filter"),minimumResultsForSearch:1/0,allowClear:!0})}var tagWaringBlock,actionButtons,actionList,$container=$(cssScope),$content=$container.find(".content"),$list=$container.find(".list"),crumbs=$container.data("breadcrumbs"),dataset=$container.data("set"),extraFields=$container.data("extrafields"),categories=$container.data("categories"),deliveryId=$container.data("delivery"),context=$container.data("context"),isManageable=$container.data("ismanageable"),timeHandlingButton=($container.data("testcenter"),
$container.data("timehandling")),defaultTag=$container.data("defaulttag"),printReportButton=$container.data("printreportbutton"),manageUrl=helpers._url("manage","Delivery","taoProctoring",{delivery:deliveryId}),terminateUrl=helpers._url("terminateExecutions","Monitor","taoProctoring",{delivery:deliveryId}),pauseUrl=helpers._url("pauseExecutions","Monitor","taoProctoring",{delivery:deliveryId}),authoriseUrl=helpers._url("authoriseExecutions","Monitor","taoProctoring",{delivery:deliveryId}),extraTimeUrl=helpers._url("extraTime","Delivery","taoProctoring",{delivery:deliveryId}),reportUrl=helpers._url("reportExecutions","Monitor","taoProctoring",{delivery:deliveryId
}),serviceUrl=helpers._url("deliveryExecutions","Monitor","taoProctoring",{delivery:deliveryId,context:context}),serviceAllUrl=helpers._url("deliveryExecutions","Monitor","taoProctoring",{context:context}),tools=[],model=[],highlightRows=[];breadcrumbsFactory($container,crumbs),tools.push({id:"refresh",icon:"reset",title:__("Refresh the page"),label:__("Refresh"),action:function(){$list.datatable("refresh")}}),defaultTag&&(tools.push({id:"notag",icon:"filter",css:"btn-warning",label:__("Remove default tag filtering"),title:__("Remove default tag filtering"),action:function(){setTagUsage(!1),$list.datatable("filter")}}),tools.push({id:"tag",icon:"filter",title:__("Apply default tag"),
label:__("Apply default tag"),action:function(){setTagUsage(!0),$list.datatable("filter")}})),deliveryId&&isManageable&&tools.push({id:"manage",icon:"property-advanced",title:__("Manage sessions"),label:__("Manage"),action:function(){window.location.href=manageUrl}}),tools.push({id:"authorise",icon:"play",title:__("Authorize sessions"),label:__("Authorise"),massAction:!0,action:authorise}),tools.push({id:"pause",icon:"pause",title:__("Pause sessions"),label:__("Pause"),massAction:!0,action:pause}),tools.push({id:"terminate",icon:"stop",title:__("Terminate sessions"),label:__("Terminate"),massAction:!0,action:terminate}),tools.push({id:"irregularity",icon:"delivery-small",
...
"use strict";var cssScope=".session-history",taoProctoringReportCtlr={start:function(){var $container=$(cssScope),dataset=$container.data("set"),deliveryId=($container.data("testcenter"),$container.data("delivery")),sessions=$container.data("sessions"),sortBy=$container.data("sortBy"),sortOrder=$container.data("sortorder"),periodStart=$container.data("periodstart"),periodEnd=$container.data("periodEnd"),serviceUrl=helpers._url("history","Reporting","taoProctoring",{delivery:deliveryId,session:sessions}),detailedHistory=!1,historyTable=historyTableFactory({tools:[{id:"show-detailed-report",icon:"insert-horizontal-line",title:__("Show detailed session history messages"),
label:__("Show detailed report"),action:function(){var tool=historyTable.config.tools.find(function(val){return"show-detailed-report"===val.id});historyTable.config.params.detailed=detailedHistory=!detailedHistory,tool.label=__(detailedHistory?"Show brief report":"Show detailed report"),historyTable.refresh()}}],params:{detailed:detailedHistory},service:serviceUrl,sortBy:sortBy,sortOrder:sortOrder},dataset).on("loading",function(){loadingBar.start()}).on("loaded",function(){loadingBar.stop()}).render($container.find(".list"));breadcrumbsFactory($container,$container.data("breadcrumbs")),dateRangeFactory({start:periodStart,end:periodEnd,renderTo:$container.find(".panel")
}).on("change submit",function(){historyTable.refresh({periodStart:this.getStart(),periodEnd:this.getEnd()})})}};return loadingBar.start(),taoProctoringReportCtlr}),define("taoProctoring/helper/contentLoader",["jquery","lodash","core/promise"],function($,_,Promise){"use strict";function isDataEmpty(data){return _.isEmpty(data)}function getContentData($container,url,checkEmpty){var data=$container.data();return checkEmpty=_.isFunction(checkEmpty)?checkEmpty:isDataEmpty,checkEmpty(data)&&url?requestData(url):Promise.resolve(data)}function requestData(url){return new Promise(function(reject,resolve){$.ajax({url:url,cache:!1,dataType:"json",type:"GET"}).done(function(response){
response&&response.success?resolve(response.data):reject(response)}).fail(reject)})}return{getContentData:getContentData,requestData:requestData}}),define("taoProctoring/helper/containerAccess",["jquery"],function($){"use strict";function getContainer(cssScope){var $container=$(containerSelector);return $container.is(cssScope)||clearContainer(cssScope,$container),$container}function clearContainer(cssScope,$container){return $container||($container=$(containerSelector)),$container.removeClass().addClass(containerCls),$(".header",$container).empty(),$(".content",$container).empty(),cssScope&&("."===cssScope.charAt(0)&&(cssScope=cssScope.substr(1)),$container.addClass(cssScope)),
$container}var containerCls="container",containerSelector="."+containerCls;return{getContainer:getContainer,clearContainer:clearContainer}}),define("taoProctoring/helper/textConverter",["jquery","lodash","core/format","core/promise","util/url"],function($,_,format,Promise,url){"use strict";function proctoringLabelsLoader(){return pendingPromise||(pendingPromise=new Promise(function(resolve,reject){$.ajax({url:url.route("getAll","TextConverter","taoProctoring"),type:"POST",dataType:"json"}).done(function(data){labels=data.translations,resolve(proctoringLabels)}).fail(reject)})),pendingPromise}var pendingPromise,labels=null,proctoringLabels={get:function(message){
void 0==labels[message]&&console.log(message);var localized=labels[message]?labels[message]:message;return arguments.length>1&&(arguments[0]=localized,localized=format.apply(null,arguments)),localized}};return proctoringLabelsLoader}),define("tpl!taoProctoring/templates/testSiteAdmin/adminLink",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<a href="',(helper=helpers.href)?stack1=helper.call(depth0,{hash:{},data:data
}):(helper=depth0&&depth0.href,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="admin-link">\n    <span class="icon-users"></span>\n    ',(helper=helpers.manageProctorMenu)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.manageProctorMenu,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"\n</a>"})}),define("taoProctoring/controller/TestCenter.bak/index",["jquery","lodash","i18n","util/url","layout/loading-bar","ui/listbox","taoProctoring/controller/app","taoProctoring/helper/contentLoader","taoProctoring/helper/containerAccess","taoProctoring/component/breadcrumbs","taoProctoring/helper/textConverter","tpl!taoProctoring/templates/testSiteAdmin/adminLink"],function($,_,__,urlHelper,loadingBar,listBox,appController,contentLoader,containerAccess,breadcrumbsFactory,textConverter,adminLinkTpl){
"use strict";var cssScope="testcenters-index";loadingBar.start();var taoProctoringIndexCtlr={start:function(){var labels,data,$container=containerAccess.getContainer(cssScope),serviceUrl=urlHelper.route("index","TestCenter","taoProctoring"),adminUrl=urlHelper.route("index","ProctorManager","taoProctoring"),list=listBox({title:__("My Test sites"),textEmpty:__("No test site available"),textNumber:__("Available"),textLoading:__("Loading"),renderTo:$container.find(".content"),replace:!0});list.setLoading(!0),Promise.all([textConverter().then(function(lbls){labels=lbls}),contentLoader.getContentData($container,serviceUrl).then(function(dat){data=dat})]).then(function(){
breadcrumbsFactory($container,data.breadcrumbs),_.forEach(data.list,function(box){box.cls=((box.cls||"")+" router").trim()}),list.update(data.list),loadingBar.stop(),data.administrator&&$container.find(".header").append(adminLinkTpl({href:adminUrl,manageProctorMenu:labels.get("Manage Proctors")}))})["catch"](function(err){appController.onError(err)})}};return taoProctoringIndexCtlr}),define("taoProctoring/controller/TestCenter.bak/testCenter",["jquery","i18n","util/url","layout/loading-bar","ui/listbox","taoProctoring/controller/app","taoProctoring/helper/contentLoader","taoProctoring/helper/containerAccess","taoProctoring/component/breadcrumbs"],function($,__,urlHelper,loadingBar,listBox,appController,contentLoader,containerAccess,breadcrumbsFactory){
"use strict";var cssScope="testcenters-testcenter";loadingBar.start();var taoProctoringTestSiteCtlr={start:function(){var $container=containerAccess.getContainer(cssScope),serviceUrl=urlHelper.route("testCenter","TestCenter","taoProctoring"),currentRoute=urlHelper.parse(window.location.href),initUrl=urlHelper.build(serviceUrl,{testCenter:decodeURIComponent(currentRoute.query.testCenter)}),list=listBox({title:"",textEmpty:!1,textNumber:!1,textLoading:__("Loading"),renderTo:$container.find(".content"),replace:!0,width:12});list.setLoading(!0),console.log("requiring",initUrl),contentLoader.getContentData($container,initUrl).then(function(data){console.log("received",data),
breadcrumbsFactory($container,data.breadcrumbs),list.setTitle(data.title),list.update(data.list),loadingBar.stop(),console.log("finished",list)})["catch"](function(err){appController.onError(err)})}};return taoProctoringTestSiteCtlr}),define("taoProctoring/controller/Tools/pauseActiveExecutions",["jquery","i18n","helpers","util/url","ui/feedback","ui/cascadingComboBox","ui/bulkActionPopup"],function($,__,helpers,url,feedback,cascadingComboBox,bulkActionPopup){"use strict";function doPause(reason){$.ajax({type:"POST",data:{reason:reason},url:url.route("pauseActiveExecutions","Tools","taoProctoring"),dataType:"json",success:function(data){helpers.loaded(),data.success?feedback().success(data.message):feedback().error(data.message);
}})}var $container=$(".js-pause-active-executions-container"),categories=$container.data("reasoncategories"),msg=__("Warning, you are about to pause all in progress tests. All test takers will be paused on or before the next heartbeat. Please provide a reason for this action.");return{start:function(){$(".js-pause").on("click",function(){var config;config={renderTo:$container,actionName:msg,reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(categories.pause)},bulkActionPopup(config).on("ok",function(reason){doPause(reason)})})}}});
