define("taoProctoring/controller/routes",{Reporting:{actions:{index:"controller/Reporting/history"}},Irregularity:{actions:{index:"controller/Irregularity/index"}},DeliverySelection:{actions:{index:"controller/Delivery/index"}},Monitor:{actions:{index:"controller/Delivery/monitoring"}},Tools:{css:"tools",actions:{assessmentActivity:"controller/Tools/assessmentActivity"}}}),define("controller/app",["jquery","core/historyRouter","core/logger","core/eventifier","core/statifier","util/url","ui/feedback"],function($,historyRouterFactory,loggerFactory,eventifier,statifier,urlUtil,feedback){"use strict";var historyRouter,appLogger=loggerFactory("controller/app"),appController=eventifier(statifier({
...
cls:"back-button",renderTo:container.find(".panel")}).on("click",function(){monitoringUrl?appController.getRouter().redirect(monitoringUrl):history.go(-1)})})}).catch(function(err){handleOnDisconnect(err),appController.onError(err),loadingBar.stop()})}}}),define("taoProctoring/controller/Tools/assessmentActivity",["jquery","i18n","helpers","util/url","ui/feedback","ui/cascadingComboBox","ui/bulkActionPopup","ui/datatable"],function($,__,helpers,url,feedback,cascadingComboBox,bulkActionPopup,datatable){"use strict";function doPause(reason){$.ajax({type:"POST",data:{reason:reason},url:url.route("pauseActiveExecutions","Tools","taoProctoring"),dataType:"json",success:function(data){
helpers.loaded(),data.success?feedback().success(data.message):feedback().error(data.message)}})}var $container=$(".js-pause-active-executions-container"),categories=$container.data("reasoncategories"),msg=__("Warning, you are about to pause all in progress tests. All test takers will be paused on or before the next heartbeat. Please provide a reason for this action.");return{start:function(){var $deliveryList=$(".js-delivery-list"),deliveryListModel=[{id:"label",label:__("Delivery"),sortable:!0},{id:"Awaiting",label:__("Awaiting"),sortable:!0,transform:function(value,row){return value.toString()}},{id:"Authorized",label:__("Authorized"),sortable:!0,transform:function(value,row){
return value.toString()}},{id:"Paused",label:__("Paused"),sortable:!0,transform:function(value,row){return value.toString()}},{id:"Active",label:__("Active"),sortable:!0,transform:function(value,row){return value.toString()}},{id:"Terminated",label:__("Terminated"),sortable:!0,transform:function(value,row){return value.toString()}},{id:"Canceled",label:__("Canceled"),sortable:!0,transform:function(value,row){return value.toString()}},{id:"Finished",label:__("Finished"),sortable:!0,transform:function(value,row){return value.toString()}}];$deliveryList.datatable({url:url.route("deliveriesActivityData","Tools","taoProctoring"),filter:!1,model:deliveryListModel,paginationStrategyTop:"none",
paginationStrategyBottom:"none",selectable:!0,sortorder:"asc",sortby:"label"},deliveryListModel),$deliveryList.datatable("refresh"),$(".js-pause").on("click",function(){var config;config={renderTo:$container,actionName:msg,reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(categories.pause)},bulkActionPopup(config).on("ok",function(reason){doPause(reason)})})}}});
