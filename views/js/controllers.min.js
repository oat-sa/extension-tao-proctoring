...
cls:"back-button",renderTo:container.find(".panel")}).on("click",function(){monitoringUrl?appController.getRouter().redirect(monitoringUrl):history.go(-1)})})})["catch"](function(err){handleOnDisconnect(err),appController.onError(err),loadingBar.stop()})}}}),define("taoProctoring/controller/Tools/pauseActiveExecutions",["jquery","i18n","helpers","util/url","ui/feedback","ui/cascadingComboBox","ui/bulkActionPopup"],function($,__,helpers,url,feedback,cascadingComboBox,bulkActionPopup){"use strict";function doPause(reason){$.ajax({type:"POST",data:{reason:reason},url:url.route("pauseActiveExecutions","Tools","taoProctoring"),dataType:"json",success:function(data){
helpers.loaded(),data.success?feedback().success(data.message):feedback().error(data.message)}})}var $container=$(".js-pause-active-executions-container"),categories=$container.data("reasoncategories"),msg=__("Warning, you are about to pause all in progress tests. All test takers will be paused on or before the next heartbeat. Please provide a reason for this action.");return{start:function(){$(".js-pause").on("click",function(){var config;config={renderTo:$container,actionName:msg,reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(categories.pause)},bulkActionPopup(config).on("ok",function(reason){doPause(reason)})})}}});
