...
}}],params:{detailed:detailedHistory},service:serviceUrl,sortBy:sortBy,sortOrder:sortOrder},dataset).on("loading",function(){loadingBar.start()}).on("loaded",function(){loadingBar.stop()}).render($container.find(".list"));breadcrumbsFactory($container,$container.data("breadcrumbs")),dateRangeFactory({start:periodStart,end:periodEnd,renderTo:$container.find(".panel")}).on("change submit",function(){historyTable.refresh({periodStart:this.getStart(),periodEnd:this.getEnd()})})}};return loadingBar.start(),taoProctoringReportCtlr});
