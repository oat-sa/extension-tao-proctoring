...
},action:function(id){dialog({message:__("The test taker will be authorized to start this delivery. Continue ?"),autoRender:!0,autoDestroy:!0,onOkBtn:function(){authorise([id])}})}},{id:"remove",icon:"remove",title:__("Remove the test taker from the delivery"),action:function(id){dialog({autoRender:!0,autoDestroy:!0,message:__("The test taker will be removed from this delivery. Continue ?"),onOkBtn:function(){remove([id])}})}}],selectable:!0,model:[{id:"firstname",label:__("First name")},{id:"lastname",label:__("Last name")},{id:"company",label:__("Company name")},{id:"status",label:__("Status")}]})}};return proctorDeliveryIndexCtlr}),define("taoProctoring/controller/ProctorDelivery/testTakers",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","ui/datatable"],function($,__,helpers,loadingBar,encode,feedback){var cssScope=".delivery-testtakers";
loadingBar.start();var proctorDeliveryAssignCtlr={start:function(){var $list=$(cssScope+" .list"),dataset=$list.data("set"),deliveryId=$list.data("id"),serviceUrl=helpers._url("availableTestTakers","ProctorDelivery","taoProctoring",{id:deliveryId}),assignUrl=helpers._url("assign","ProctorDelivery","taoProctoring",{id:deliveryId}),indexUrl=helpers._url("index","ProctorDelivery","taoProctoring",{id:deliveryId}),assign=function(selection){selection&&selection.length&&(loadingBar.start(),$.ajax({url:assignUrl,data:{tt:selection},dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){loadingBar.stop(),response&&response.success?(feedback().success(__("Test takers have been added")),location.href=indexUrl):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})
}))};$list.on("query.datatable",function(){loadingBar.start()}).on("load.datatable",function(){loadingBar.stop()}).datatable({url:serviceUrl,data:dataset,status:{empty:__("No available test takers to assign"),available:__("Available test takers"),loading:__("Loading")},tools:[{id:"back",icon:"left",title:__("Return to the delivery"),label:__("Back"),action:function(){history.back()}},{id:"assign",icon:"add",title:__("Assign the selected test takers to the delivery"),label:__("Assign the selected test takers"),massAction:!0,action:function(selection){assign(selection)}}],actions:[{id:"assign",icon:"add",title:__("Assign the test taker to the delivery"),action:function(id){assign([id])
}}],selectable:!0,model:[{id:"firstname",label:__("First name")},{id:"lastname",label:__("Last name")},{id:"company",label:__("Company name")}]})}};return proctorDeliveryAssignCtlr}),define("tpl!taoProctoring/tpl/entryPoint",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n    <li>\n        <a class="block entry-point" href="',(helper=helpers.url)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.url,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n            <h3>',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</h3>\n            ",stack1=helpers["if"].call(depth0,depth0&&depth0.content,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n            <div class="clearfix">\n                <span class="text-link" href="',(helper=helpers.url)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.url,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"><span class="icon-play"></span>',(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n            </div>\n        </a>\n    </li>\n    "
...
})}),define("taoProctoring/controller/TaoProctoring/index",["jquery","helpers","layout/loading-bar","tpl!taoProctoring/tpl/entryPoint"],function($,helpers,loadingBar,entryPointTpl){var refreshPolling=6e4,cssScope=".delivery-index",hiddenCls="hidden";loadingBar.start();var taoProctoringCtlr={start:function(){var $titleLoading=$(cssScope+" .loading"),$titleEmpty=$(cssScope+" .empty-list"),$titleAvailable=$(cssScope+" .available-list"),$titleCount=$(cssScope+" .count"),$list=$(cssScope+" .list"),listEntries=$list.data("list"),serviceUrl=helpers._url("deliveries","TaoProctoring","taoProctoring"),pollTo=null,update=function(entries){pollTo&&(clearTimeout(pollTo),pollTo=null),$titleLoading.addClass(hiddenCls),$list.empty(),entries&&entries.length?($titleAvailable.removeClass(hiddenCls),$list.append(entryPointTpl({entries:entries})),$titleCount.text(entries.length),$titleEmpty.addClass(hiddenCls)):($titleEmpty.removeClass(hiddenCls),$titleAvailable.addClass(hiddenCls)),loadingBar.stop(),refreshPolling&&(pollTo=setTimeout(refresh,refreshPolling))
},refresh=function(){loadingBar.start(),$titleLoading.removeClass(hiddenCls),$.ajax({url:serviceUrl,data:{_cb:Date.now()},dataType:"json",type:"GET"}).done(function(response){listEntries=response&&response.entries,update(listEntries)})};listEntries?update(listEntries):refresh()}};return taoProctoringCtlr});
