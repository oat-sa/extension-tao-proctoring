...
data.success?feedback().success(data.message):feedback().error(data.message)}})}var $container=$(".js-pause-active-executions-container"),categories=$container.data("reasoncategories"),msg=__("Warning, you are about to pause all in progress tests. All test takers will be paused on or before the next heartbeat. Please provide a reason for this action.");return{start:function(){$(".js-pause").on("click",function(){var config;config={renderTo:$container,actionName:msg,reason:!0,allowedResources:[],reasonRequired:!0,categoriesSelector:cascadingComboBox(categories.pause)},bulkActionPopup(config).on("ok",function(reason){doPause(reason)})})}}});
